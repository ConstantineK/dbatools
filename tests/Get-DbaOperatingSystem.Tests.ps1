...
Describe "Get-OperatingSystem Unit Tests" -Tag "UnitTests" {
    InModuleScope sqlshell{
        Context "Validate parameters" {
            $params = (Get-ChildItem function:\Get-OperatingSystem).Parameters
        ...
        Context "Validate input" {
            it "Cannot resolve hostname of computer" {
                ...
                {Get-OperatingSystem -ComputerName 'DoesNotExist142' -WarningAction Stop 3> $null} | Should Throw
            }
        }
    }
}
Describe "Get-OperatingSystem Integration Test" -Tag "IntegrationTests" {
    $result = Get-OperatingSystem -ComputerName $script:instance1
    ...
    Context "Validate output" {
        ...
        it "Should return nothing if unable to connect to server" {
            $result = Get-OperatingSystem -ComputerName 'Melton5312' -WarningAction SilentlyContinue
