...
Describe "$CommandName Unit Tests" -Tag 'UnitTests' {
    Context "Validate parameters" {
        ...
        [object[]]$params = (Get-ChildItem function:\Get-SqlInstanceProperty).Parameters.Keys
...
Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
    Context "Command actually works" {
        $results = Get-SqlInstanceProperty -SqlInstance $script:instance2
        ...
        It "Should return that returns a valid build" {
            $(Get-SqlBuildReference -Build ($results | Where-Object {$_.name -eq 'ResourceVersionString'}).Value).MatchType | Should Be "Exact"
        }
        ...
        It "Should get the correct DefaultFile location" {
            $defaultFiles = Get-DefaultPath -SqlInstance $script:instance2
    ...
    Context "Command can handle multiple instances" {
        It "Should have results for 2 instances" {
            $(Get-SqlInstanceProperty -SqlInstance $script:instance1, $script:instance2 | Select-Object -unique SqlInstance).count | Should Be 2
        }
    }
}