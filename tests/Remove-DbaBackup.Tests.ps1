...
Describe "$CommandName Unit Tests" -Tag 'UnitTests' {
    Context "Validate parameters" {
        ...
        [object[]]$params = (Get-ChildItem function:\Remove-Backup).Parameters.Keys
        It "Should only contain $paramCount parameters" {
            $params.Count - $defaultParamCount | Should Be $paramCount
        }
    }
    Context "It's Confirm impact should be medium" {
        $command = Get-Command Remove-Backup
...
Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
    ...
    Context "Path validation" {
        { Remove-Backup -Path 'funnypath' -BackupFileExtension 'bak' -RetentionPeriod '0d' -EnableException } | Should Throw "not found"
    }
    Context "RetentionPeriod validation" {
        { Remove-Backup -Path $testPath -BackupFileExtension 'bak' -RetentionPeriod 'ad' -EnableException } | Should Throw "format invalid"
        { Remove-Backup -Path $testPath -BackupFileExtension 'bak' -RetentionPeriod '11y' -EnableException } | Should Throw "units invalid"
    }
    Context "BackupFileExtension validation" {
        { Remove-Backup -Path $testPath -BackupFileExtension '.bak' -RetentionPeriod '0d' -EnableException } | Should Not Throw
    }
    Context "BackupFileExtension message validation" {
        $warnmessage = Remove-Backup -Path $testPath -BackupFileExtension '.bak' -RetentionPeriod '0d' 3>&1
    ...
    Context "Files are removed" {
        ...
        It "Should remove all files with retention 0d" {
            $null = Remove-Backup -Path $testPath -BackupFileExtension 'bak' -RetentionPeriod '0d'
    ...
    Context "Cleanup empty folders" {
        ...
        It "Removes files but leaves empty dirs" {
            Remove-Backup -Path $testPath -BackupFileExtension 'bak' -RetentionPeriod '0d'
            (Get-ChildItem -Path $testPath -Directory -Recurse).Count | Should Be 2
        }
        It "Removes files and removes empty dirs" {
            Remove-Backup -Path $testPath -BackupFileExtension 'bak' -RetentionPeriod '0d' -RemoveEmptyBackupFolder
