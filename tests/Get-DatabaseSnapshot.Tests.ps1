...
Describe "$commandname Integration Tests" -Tags "IntegrationTests" {
    Context "Operations on snapshots" {
        BeforeAll {
            ...
            $setupright = $true
            $needed = @()
            $needed += New-DatabaseSnapshot -SqlInstance $script:instance2 -Database $db1 -Name $db1_snap1 -WarningAction SilentlyContinue
            $needed += New-DatabaseSnapshot -SqlInstance $script:instance2 -Database $db1 -Name $db1_snap2 -WarningAction SilentlyContinue
            $needed += New-DatabaseSnapshot -SqlInstance $script:instance2 -Database $db2 -Name $db2_snap1 -WarningAction SilentlyContinue
        ...
        if ($setupright) {
            It "Gets all snapshots by default" {
                $results = Get-DatabaseSnapshot -SqlInstance $script:instance2
            ...
            It "Honors the Database parameter, returning only snapshots of that database" {
                $results = Get-DatabaseSnapshot -SqlInstance $script:instance2 -Database $db1
                $results.Count | Should Be 2
                $result = Get-DatabaseSnapshot -SqlInstance $script:instance2 -Database $db2
            ...
            It "Honors the ExcludeDatabase parameter, returning relevant snapshots" {
                ...
                $results = Get-DatabaseSnapshot -SqlInstance $script:instance2 -ExcludeDatabase $alldbs
            ...
            It "Honors the ExcludeSnapshot parameter" {
                $result = Get-DatabaseSnapshot -SqlInstance $script:instance2 -ExcludeSnapshot $db1_snap1 -Database $db1
