...
Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
    Context "Verifying User is removed" {
        BeforeAll {
            ...
            $db = Get-Database $server -Database tempdb
            $securePassword = ConvertTo-SecureString "password" -AsPlainText -Force
            $loginTest = New-Login $server -Login dbatoolsci_remove_dba_db_user -Password $securePassword
        }
        BeforeEach {
        ...
        It "drops a user with no ownerships" {
            Remove-DbUser $server -Database tempdb -User $user.Name
        ...
        It "does NOT drop a user that owns objects other than a schema" {
            ...
            $table.Create()
            Remove-DbUser $server -Database tempdb -User $user.Name -WarningAction SilentlyContinue
