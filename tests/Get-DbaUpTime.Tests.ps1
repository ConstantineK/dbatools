...
Describe "$CommandName Unit Tests" -Tag 'UnitTests' {
    Context "Validate parameters" {
        ...
        [object[]]$params = (Get-ChildItem function:\Get-Uptime).Parameters.Keys
...
Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
    Context "Command actually works" {
        $results = Get-Uptime -SqlInstance $script:instance1
    ...
    Context "Command can handle multiple SqlInstances" {
        $results = Get-Uptime -SqlInstance $script:instance1,$script:instance2
