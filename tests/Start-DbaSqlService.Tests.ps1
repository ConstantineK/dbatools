...
Describe "$commandname Integration Tests" -Tags "IntegrationTests" {
    ...
    Context "Command actually works" {
        ...
        It "starts the services back" {
            $services = Start-SqlService -ComputerName $script:instance2 -Type Agent -InstanceName $instanceName
        ...
        It "starts the services back through pipeline" {
            $services = Get-SqlService -ComputerName $script:instance2 -InstanceName $instanceName -Type Agent, Engine | Start-SqlService
            $services | Should Not Be $null
            foreach ($service in $services) {
                $service.State | Should Be 'Running'
                $service.Status | Should Be 'Successful'
        ...
        It "errors when passing an invalid InstanceName" {
            { Start-SqlService -ComputerName $script:instance2 -Type 'Agent' -InstanceName 'ThisIsInvalid' -EnableException } | Should Throw 'No SQL Server services found with current parameters.'
        }
    }
}