...
Describe "$commandname Integration Tests" -Tags "IntegrationTests" {
    ...
    Context "Parameters validation" {
        It "Stops if no Database or Snapshot" {
            { Restore-FromDatabaseSnapshot -SqlInstance $script:instance2 -EnableException } | Should Throw "You must specify"
        }
        It "Is nice by default" {
            { Restore-FromDatabaseSnapshot -SqlInstance $script:instance2 *> $null } | Should Not Throw "You must specify"
        }
    }
    Context "Operations on snapshots" {
        ...
        if ($setupright) {
            It "Honors the Database parameter, restoring only snapshots of that database" {
                $result = Restore-FromDatabaseSnapshot -SqlInstance $script:instance2 -Database $db2 -Force
                ...
                $result = Restore-FromDatabaseSnapshot -SqlInstance $script:instance2 -Database $db1 -Force
                $result.Status | Should Be "Restored"
                $result.Snapshot | Should Be $db1_snap2
                $result.Database | Should Be $db1
                # the other snapshot has been dropped
                $result = Get-DatabaseSnapshot -SqlInstance $script:instance2 -Database $db1
            ...
            It "Honors the Snapshot parameter" {
                $result = Restore-FromDatabaseSnapshot -SqlInstance $script:instance2 -Snapshot $db1_snap1 -Force
                $result.Database | Should Be $db1
                $result.Status | Should Be "Restored"
            ...
            It "Stops if multiple snapshot for the same db are passed" {
                $result = Restore-FromDatabaseSnapshot -SqlInstance $script:instance2 -Snapshot $db1_snap1, $db1_snap2 -Force *> $null
