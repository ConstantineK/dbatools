...
Describe "$CommandName Unit Tests" -Tag 'UnitTests' {
    Context "Validate parameters" {
        ...
        [object[]]$params = (Get-ChildItem function:\Test-SqlPath).Parameters.Keys
...
Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
    BeforeAll {
        $trueTest = (Get-DatabaseFile -SqlInstance $script:instance2 -Database master)[0].PhysicalName
    ...
    Context "Command actually works" {
        $result = Test-SqlPath -SqlInstance $script:instance2 -Path $trueTest
        It "Should only return true if the path IS accessible to the instance" {
            $result | Should Be $true
        }

        $result = Test-SqlPath -SqlInstance $script:instance2 -Path $falseTest
