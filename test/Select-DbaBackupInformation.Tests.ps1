...
Describe "$commandname Unit Tests" -Tag 'UnitTests' {
    InModuleScope sqlshell{
        Context "Empty TLog Backup Issues" {
            ...
            $Output = Select-BackupInformation -BackupHistory $header #-EnableException:$true
        ...
        Context "General Diff Restore from Pipeline" {
            $Header = ConvertFrom-Json -InputObject (Get-Content $PSScriptRoot\..\tests\ObjectDefinitions\BackupRestore\RawInput\DiffRestore.json -raw)
            $header | Add-Member -Type NoteProperty -Name FullName -Value 1
            $Output = $Header | Select-BackupInformation -EnableException:$true
        ...
        Context "General Diff Restore from Pipeline with IgnoreDiff" {
            $Header = ConvertFrom-Json -InputObject (Get-Content $PSScriptRoot\..\tests\ObjectDefinitions\BackupRestore\RawInput\DiffRestore.json -raw)
            $header | Add-Member -Type NoteProperty -Name FullName -Value 1
            $Output = $Header | Select-BackupInformation -EnableException:$true -IgnoreDiff
        ...
        Context "General Diff Restore from Pipeline with IgnoreLog" {
            $Header = ConvertFrom-Json -InputObject (Get-Content $PSScriptRoot\..\tests\ObjectDefinitions\BackupRestore\RawInput\DiffRestore.json -raw)
            $header | Add-Member -Type NoteProperty -Name FullName -Value 1
            $Output = $Header | Select-BackupInformation -EnableException:$true -IgnoreLogs
        ...
        Context "Server/database names and file paths have commas and spaces" {
            ...
            $Output = Select-BackupInformation -BackupHistory $header
        ...
        Context "Overlapping Diff and log Restore" {
            ...
            $Output = Select-BackupInformation  -BackupHistory $Header -RestoreTime $RestoreDate
        ...
        Context "Continue Points" {
            $BackupInfo = Get-BackupInformation -Import -Path $PSScriptRoot\..\tests\ObjectDefinitions\BackupRestore\RawInput\ContinuePointTest.xml
            ...
            $Output = Select-BackupInformation -BackupHistory $BackupInfo -EnableException:$true -ContinuePoints $ContinuePoints
