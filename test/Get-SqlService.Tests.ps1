...
Describe "$commandname Integration Tests" -Tags "IntegrationTests" {
    ...
    Context "Command actually works" {
        ...
        $results = Get-SqlService -ComputerName $script:instance2

        It "shows some services" {
            $results.DisplayName | Should Not Be $null
        }

        $results = Get-SqlService -ComputerName $script:instance2 -Type Agent
        ...
        $results = Get-SqlService -ComputerName $script:instance2 -InstanceName $instanceName -Type Agent

        It "shows a service from a specific instance" {
            $results.ServiceType| Should Be "Agent"
        }

        $service = Get-SqlService -ComputerName $script:instance2 -Type Agent -InstanceName $instanceName

        It "sets startup mode of the service to 'Manual'" {
            { $service.ChangeStartMode('Manual') } | Should Not Throw
        }

        $results = Get-SqlService -ComputerName $script:instance2 -Type Agent -InstanceName $instanceName
