...
Describe "$commandname Unit Tests" -Tags "UnitTests" {
    Context "Validating Database Input" {
        Invoke-DatabaseCorruption -SqlInstance $script:instance2 -Database "master" -WarningAction SilentlyContinue -WarningVariable systemwarn
        ...
        It "Should fail if more than one database is specified" {
            { Invoke-DatabaseCorruption -SqlInstance $script:instance2 -Database "Database1", "Database2" -EnableException } | Should Throw
        }
    }

    Context "It's Confirm impact should be high" {
        $command = Get-Command Invoke-DatabaseCorruption
...
Describe "$commandname Integration Tests" -Tags "IntegrationTests" {
    ...
    It "Require at least a single table in the database specified" {
        { Invoke-DatabaseCorruption -SqlInstance $server -Database $dbname -EnableException } | Should Throw
    }

    # Creating a table to make sure these are failing for different reasons
    It "Fail if the specified table does not exist" {
        { Invoke-DatabaseCorruption -SqlInstance $server -Database $dbname -Table "DoesntExist$(New-Guid)" -EnableException } | Should Throw
    }
    ...
    It "Corrupt a single database" {
        Invoke-DatabaseCorruption -SqlInstance $script:instance2 -Database $dbname -Confirm:$false | Select-Object -ExpandProperty Status | Should be "Corrupted"
    }
