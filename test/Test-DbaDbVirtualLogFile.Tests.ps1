...
Describe "$CommandName Unit Tests" -Tag 'UnitTests' {
    Context "Validate parameters" {
        ...
        [object[]]$params = (Get-ChildItem function:\Test-DbVirtualLogFile).Parameters.Keys
...
Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
    BeforeAll {
        ...
        $needed = Get-Database -SqlInstance $script:instance2 -Database $db1
        $setupright = $true
    ...
    Context "Command actually works" {
        $results = Test-DbVirtualLogFile -SqlInstance $script:instance2 -Database $db1
