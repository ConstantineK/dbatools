...
Describe "$CommandName Unit Tests" -Tag 'UnitTests' {
    Context "Validate parameters" {
        ...
        [object[]]$params = (Get-ChildItem function:\Get-WaitingTask).Parameters.Keys
...
Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
    ...
    Start-Job -Name $job -ScriptBlock {
        ...
        (Connect-Instance -SqlInstance $args[1] -ClientName dbatools-waiting).Query($args[2])
    ...
    $process = Get-Process -SqlInstance $instance | Where-Object Program -eq 'dbatools-waiting' | Select-Object -ExpandProperty Spid

    if ($process -ne $null) {
        Context "Command actually works" {
            $results = Get-WaitingTask -SqlInstance $instance -Spid $process
        ...
        $isProcess = Get-Process -SqlInstance $instance -Spid $process
        if ($isProcess) {
            Stop-Process -SqlInstance $instance -Spid $process

            # I've had a few cases where first run didn't actually kill the process
            $isProcess = Get-Process -SqlInstance $instance -Spid $process
            if ($isProcess) {
                Stop-Process -SqlInstance $instance -Spid $process -ErrorAction SilentlyContinue
            }
        }
