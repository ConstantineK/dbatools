...
Describe "$commandname Integration Tests" -Tags "IntegrationTests" {
    Context "Get configuration" {
        ...
        It "returns equal to or more results than the straight T-SQL query" {
            $results = Get-SpConfigure -SqlInstance $script:instance1
            $results.count -ge $configs.count
        }

        It "returns two results" {
            $results = Get-SpConfigure -SqlInstance $script:instance1 -ConfigName RemoteQueryTimeout, AllowUpdates
            $results.Count | Should Be 2
        }

        It "matches the output of sp_configure " {
            $results = Get-SpConfigure -SqlInstance $script:instance1 -ConfigName RemoteQueryTimeout
