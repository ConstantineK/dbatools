...
Describe "$commandname Integration Tests" -Tags "IntegrationTests" {
    ...
    Context "Command actually works" {
        ...
        It "stops some services" {
            $services = Stop-SqlService -ComputerName $script:instance2 -InstanceName $instanceName -Type Agent
        ...
        It "stops specific services based on instance name through pipeline" {
            $services = Get-SqlService -ComputerName $script:instance2 -InstanceName $instanceName -Type Agent, Engine | Stop-SqlService
            $services | Should Not Be $null
            foreach ($service in $services) {
                $service.State | Should Be 'Stopped'
                $service.Status | Should Be 'Successful'
