...
Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
    Context "New Agent Job Category is changed properly" {
        ...
        It "Should actually for sure exist" {
            $newresults = Get-AgentJobCategory -SqlInstance $script:instance2 -Category CategoryTest1
            $newresults.Name | Should Be "CategoryTest1"
            $newresults.CategoryType | Should Be "LocalJob"
        }

        It "Change the name of the job category" {
            $results = Set-AgentJobCategory -SqlInstance $script:instance2 -Category CategoryTest1 -NewName CategoryTest2
            $results.NewCategoryName | Should Be "CategoryTest2"
        }

        # Cleanup and ignore all output
        Remove-AgentJobCategory -SqlInstance $script:instance2 -Category CategoryTest2 *> $null
    }
}