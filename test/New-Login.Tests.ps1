...
Describe "$commandname Integration Tests" -Tags "IntegrationTests" {
    ...
    #create credential
    $null = New-Credential -SqlInstance $server1 -Name $credLogin -CredentialIdentity $credLogin -Password $securePassword -Force

    #create master key if not exists
    if (!($mkey = Get-DatabaseMasterKey -SqlInstance $server1 -Database master)) {
        $null = New-DatabaseMasterKey -SqlInstance $server1 -Database master -Password $securePassword -Confirm:$false
    }
    ...
    $null = New-DbCertificate $server1 -Name $certificateName -Password $null

    Context "Create new logins" {
        It "Should be created successfully - Hashed password" {
            $results = New-Login -SqlInstance $server1 -Login tester -HashedPassword (Get-PasswordHash $securePassword $server1.VersionMajor)
        ...
        It "Should be created successfully - password, credential and a custom sid " {
            $results = New-Login -SqlInstance $server1 -Login claudio -Password $securePassword -Sid $sid -MapToCredential $credLogin
        ...
        It "Should be created successfully - password and all the flags" {
            $results = New-Login -SqlInstance $server1 -Login port -Password $securePassword -PasswordPolicy -PasswordExpiration -DefaultDatabase tempdb -Disabled -Language Nederlands
        ...
        It "Should be created successfully - Windows login" {
            $results = New-Login -SqlInstance $server1 -Login $winLogin
        ...
        It "Should be created successfully - certificate" {
            $results = New-Login -SqlInstance $server1 -Login certifico -MapToCertificate $certificateName
        ...
        It "Should be copied successfully" {
            $results = Get-Login -SqlInstance $server1 -Login tester | New-Login -SqlInstance $server2 -Disabled:$false -Force
            $results.Name | Should Be "tester"

            $results = Get-Login -SqlInstance $server1 -Login claudio, port | New-Login -SqlInstance $server2 -Force -PasswordPolicy -PasswordExpiration -DefaultDatabase tempdb -Disabled -Language Nederlands -NewSid -LoginRenameHashtable @{claudio = 'port'; port = 'claudio'} -MapToCredential $null
            $results.Name | Should Be @("port", "claudio")

            $results = Get-Login -SqlInstance $server1 -Login tester | New-Login -SqlInstance $server1 -LoginRenameHashtable @{tester = 'port'} -Force -NewSid
            $results.Name | Should Be "port"
        }
        ...
        It "Should not have same properties because of the overrides" {
            ...
            $login1 = Get-login -SqlInstance $script:instance1 -login claudio
            $login2 = Get-login -SqlInstance $script:instance2 -login port
    ...
    Context "No overwrite" {
        $null = Get-Login -SqlInstance $server1 -Login tester | New-Login -SqlInstance $server2 -WarningVariable warning 3>&1
    ...
    if (!$mkey) {
        $null = Remove-DatabaseMasterKey -SqlInstance $script:instance1 -Database master -Confirm:$false
    }
}
