...
Describe "$CommandName Unit Tests" -Tag 'UnitTests' {
    Context "Validate parameters" {
        ...
        [object[]]$params = (Get-ChildItem function:\Get-AgentJobHistory).Parameters.Keys
...
Describe "$CommandName Unittests" -Tag 'UnitTests' {
    InModuleScope 'dbatools' {
        ...
        Context "Return values" {
            ...
            Mock Get-AgentJobOutputFile -MockWith {
                @(
                    @{
                        Job            = 'Job1'
                        StepId         = 1
            ...
            It "Throws when NoJobSteps and WithOutputFile" {
                { Get-AgentJobHistory -SqlInstance 'SQLServerName' -NoJobSteps -WithOutputFile -EnableException } | Should Throw
            }
            It "Returns full history by default" {
                $Results = @()
                $Results += Get-AgentJobHistory -SqlInstance 'SQLServerName'
                $Results.Length | Should Be 6
            }
            It "Returns only runs with no steps with NoJobSteps" {
                $Results = @()
                $Results += Get-AgentJobHistory -SqlInstance 'SQLServerName' -NoJobSteps
                $Results.Length | Should Be 2
            }
            It "Figures out plain outputfiles" {
                $Results = @()
                $Results += Get-AgentJobHistory -SqlInstance 'SQLServerName' -WithOutputFile
