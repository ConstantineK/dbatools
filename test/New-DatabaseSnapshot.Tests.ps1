...
Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
    Context "Parameter validation" {
        It "Stops if no Database or AllDatabases" {
            { New-DatabaseSnapshot -SqlInstance $script:instance2 -EnableException } | Should Throw "You must specify"
        }
        It "Is nice by default" {
            { New-DatabaseSnapshot -SqlInstance $script:instance2 *> $null } | Should Not Throw "You must specify"
        }
    }

    Context "Operations on not supported databases" {
        It "Doesn't support model, master or tempdb" {
            $result = New-DatabaseSnapshot -SqlInstance $script:instance2 -EnableException -Database model, master, tempdb
    ...
    Context "Operations on databases" {
        BeforeAll {
            ...
            $needed = Get-Database -SqlInstance $script:instance2 -Database $db1, $db2, $db3
            $setupright = $true
        ...
        AfterAll {
            Remove-DatabaseSnapshot -SqlInstance $script:instance2 -Database $db1, $db2, $db3 -Force
            Remove-Database -Confirm:$false -SqlInstance $script:instance2 -Database $db1, $db2, $db3
        }
        ...
        if ($setupright) {
            if (-not $env:appveyor) {
                It "Skips over offline databases nicely" {
                    ...
                    $result = New-DatabaseSnapshot -SqlInstance $script:instance2 -EnableException -Database $db3
                    $result | Should Be $null
            ...
            It "Refuses to accept multiple source databases with a single name target" {
                { New-DatabaseSnapshot -SqlInstance $script:instance2 -EnableException -Database $db1, $db2 -Name "dbatools_Snapped" } | Should Throw
            }
            It "Halts when path is not accessible" {
                { New-DatabaseSnapshot -SqlInstance $script:instance2 -Database $db1 -Path B:\Funnydbatoolspath -EnableException } | Should Throw
            }
            It "Creates snaps for multiple dbs by default" {
                $result = New-DatabaseSnapshot -SqlInstance $script:instance2 -EnableException -Database $db1, $db2
            ...
            It "Creates snap with the correct name" {
                $result = New-DatabaseSnapshot -SqlInstance $script:instance2 -EnableException -Database $db1 -Name "dbatools_SnapMe_right"
                $result | Should Not Be $null
            ...
            It "Creates snap with the correct name template" {
                $result = New-DatabaseSnapshot -SqlInstance $script:instance2 -EnableException -Database $db2 -NameSuffix "dbatools_SnapMe_{0}_funny"
                $result | Should Not Be $null
            ...
            It "Has the correct properties" {
                ...
                $result = New-DatabaseSnapshot -SqlInstance $script:instance2 -EnableException -Database $db2
