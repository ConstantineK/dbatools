...
Describe "$CommandName Unit Tests" -Tag 'UnitTests' {
    Context "Validate parameters" {
        ...
        [object[]]$params = (Get-ChildItem Function:\Test-SqlManagementObject).Parameters.Keys
...
Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
    BeforeAll {
        $versionMajor = (Connect-Instance -SqlInstance $script:instance2).VersionMajor
    }
    Context "Command actually works" {
        $trueResults = Test-SqlManagementObject -ComputerName $script:instance2 -VersionNumber $versionMajor
        ...
        $falseResults = Test-SqlManagementObject -ComputerName $script:instance2 -VersionNumber -1
