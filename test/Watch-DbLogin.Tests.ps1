...
Describe "$CommandName Unit Tests" -Tag 'UnitTests' {
    Context "Validate parameters" {
        ...
        [object[]]$params = (Get-ChildItem function:\Watch-DbLogin).Parameters.Keys
...
Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
    ...
    AfterAll {
        $null = (Connect-Instance -SqlInstance $script:instance1).Databases[$databaseName].Query("DROP TABLE $tableName")
    }
    Context "Command actually works" {
        Watch-DbLogin -SqlInstance $script:instance1 -Database $databaseName -Table $tableName -ServersFromFile $testFile -EnableException
        $result = Get-Table -SqlInstance $script:instance1 -Database $databaseName -Table $tableName -IncludeSystemDBs
