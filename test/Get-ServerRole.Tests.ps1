...
Describe "$CommandName Unit Tests" -Tag 'UnitTests' {
    Context "Validate parameters" {
        ...
        [object[]]$params = (Get-ChildItem function:\Get-ServerRole).Parameters.Keys
    ...
    Context "Input validation" {
        ...
        It "Should Call Stop-Function if instance does not exist or connection failure" {
            Set-Config -FullName sql.connection.timeout -Value 1
            Get-ServerRole -SqlInstance Dummy | Should Be
        }
...
Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
    Context "Command actually works" {
        $results = Get-ServerRole -SqlInstance $script:instance2
        ...
        It "Shows only one value with ServerRole parameter" {
            $results = Get-ServerRole -SqlInstance $script:instance2 -ServerRole sysadmin
            $results[0].Role | Should Be "sysadmin"
        }

        It "Should exclude sysadmin from output" {
            $results = Get-ServerRole -SqlInstance $script:instance2 -ExcludeServerRole sysadmin
            'sysadmin' -NotIn $results.Role | Should Be $true
        }

        It "Should exclude fixed server-level roles" {
            $results = Get-ServerRole -SqlInstance $script:instance2 -ExcludeFixedRole
            'sysadmin' -NotIn $results.Role | Should Be $true
        }
