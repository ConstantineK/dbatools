...
Describe "$commandname Integration Tests" -Tags "IntegrationTests" {
    ...
    BeforeAll {
        ...
        $null = Get-Database -SqlInstance $script:instance1 -Database $dbname2 | Remove-Database -Confirm:$false
        $null = Restore-Database -SqlInstance $script:instance1 -Path $script:appveyorlabrepo\singlerestore\singlerestore.bak -DatabaseName $dbname2 -DestinationFilePrefix $dbname2
        $db2 = Get-Database -SqlInstance $script:instance1 -Database $dbname2
        $db2 | Backup-Database -Type Full -BackupDirectory $DestBackupDir
        $db2 | Backup-Database -Type Differential -BackupDirectory $DestBackupDir
        $db2 | Backup-Database -Type Log -BackupDirectory $DestBackupDir
        ...
        $null = Get-Database -SqlInstance $script:instance1 -Database $dbname3 | Remove-Database -Confirm:$false
        $null = Restore-Database -SqlInstance $script:instance1 -Path $script:appveyorlabrepo\singlerestore\singlerestore.bak -DatabaseName $dbname3 -DestinationFilePrefix $dbname3
        $db3 = Get-Database -SqlInstance $script:instance1 -Database $dbname3
        $db3 | Backup-Database -Type Full -BackupDirectory "$DestBackupDirOla\FULL"
        $db3 | Backup-Database -Type Differential -BackupDirectory "$DestBackupDirOla\Diff"
        $db3 | Backup-Database -Type Log -BackupDirectory "$DestBackupDirOla\LOG"
    }
    ...
    Context "Get history for all database" {
        $results = Get-BackupInformation -SqlInstance $script:instance1 -Path $DestBackupDir
    ...
    Context "Get history for one database" {
        $results = Get-BackupInformation -SqlInstance $script:instance1 -Path $DestBackupDir -DatabaseName $dbname2
    ...
    Context "Check the export/import of backup history" {
        ...
        $results = Get-BackupInformation -SqlInstance $script:instance1 -Path $DestBackupDir -DatabaseName $dbname2 -ExportPath "$DestBackupDir\history.xml"

        # the command below returns just a warning
        # Get-BackupInformation -Import -Path "$DestBackupDir\history.xml" | Restore-Database -SqlInstance $script:instance1 -DestinationFilePrefix hist -RestoredDatabaseNamePrefix hist -TrustDbBackupHistory
    ...
    Context "Test Maintenance solution options" {
        $results = Get-BackupInformation -SqlInstance $script:instance1 -Path $DestBackupDirOla -MaintenanceSolution
        It "Should be 3 backups returned" {
            $results.count | Should Be 3
        }
        It "Should Be 1 full backup" {
        ...
        $ResultsSanLog = Get-BackupInformation -SqlInstance $script:instance1 -Path $DestBackupDirOla -MaintenanceSolution -IgnoreLogBackup
        ...
        $ResultsSanLog = Get-BackupInformation -SqlInstance $script:instance1 -Path $DestBackupDirOla -IgnoreLogBackup -WarningVariable warnvar -WarningAction SilentlyContinue
