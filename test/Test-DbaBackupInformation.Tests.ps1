...
Describe "$commandname Unit Tests" -Tag 'UnitTests' {
    InModuleScope sqlshell{
        Context "Everything as it should" {
            ...
            $BackupHistory = $BackupHistory | Format-BackupInformation
            ...
            Mock Get-Database { $null }

            Mock New-SqlDirectory {$true}
            Mock Test-SqlPath { [pscustomobject]@{
            ...
            It "Should pass as all systems Green" {
                $output = $BackupHistory | Test-BackupInformation -SqlServer NotExist -WarningVariable warnvar -WarningAction SilentlyContinue
        ...
        Context "Pass if Destination db exists and WithReplace set" {
            ...
            It "Should pass if destdb exists and WithReplace specified" {
                $output = $BackupHistory | Test-BackupInformation -SqlServer NotExist -WarningVariable warnvar -WarningAction SilentlyContinue -WithReplace
                ($output.Count) -gt 0 | Should be $true
                $true -in ($Output.IsVerified) | Should be $False
                ($null -ne $WarnVar) | Should be $True
            }
