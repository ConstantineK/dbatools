...
Describe "$CommandName Unit Tests" -Tag 'UnitTests' {
    Context "Validate parameters" {
        ...
        [object[]]$params = (Get-ChildItem function:\Get-AgentJobOutputFile).Parameters.Keys
    ...
    InModuleScope 'dbatools' {
        Context "Return values" {
            ...
            It "Gets only steps with output files" {
                $Results = @()
                $Results += Get-AgentJobOutputFile -SqlInstance 'SQLServerName'
            ...
            It "Honors the Job parameter" {
                $Results = @()
                $Results += Get-AgentJobOutputFile -SqlInstance 'SQLServerName' -Job 'Job1'
            ...
            It "Honors the ExcludeJob parameter" {
                $Results = @()
                $Results += Get-AgentJobOutputFile -SqlInstance 'SQLServerName' -ExcludeJob 'Job1'
            ...
            It "Does not return even with a specific job without outputfiles" {
                $Results = @()
                $Results += Get-AgentJobOutputFile -SqlInstance 'SQLServerName' -Job 'Job3'
