...
Describe "Get-ComputerSystem Unit Tests" -Tag "UnitTests" {
    InModuleScope sqlshell{
        Context "Validate parameters" {
            $params = (Get-ChildItem function:\Get-ComputerSystem).Parameters
...
Describe "Get-ComputerSystem Integration Test" -Tag "IntegrationTests" {
    $result = Get-ComputerSystem -ComputerName $script:instance1
    ...
    Context "Validate output" {
        ...
        it "Should return nothing if unable to connect to server" {
            $result = Get-ComputerSystem -ComputerName 'Melton5312' -WarningAction SilentlyContinue
