...
Describe "$commandname Integration Tests" -Tags "IntegrationTests" {
    ...
    if ($l = Get-Login -SqlInstance $script:instance2 -Login $winLogin) {
    ...
    $services = Get-Sqlservice -ComputerName $script:instance2 -Type Engine, Agent -Instance $instanceName
    ...
    Context "Set new service account for SQL Services" {
        ...
        $results = Update-SqlServiceAccount -ComputerName $computerName -ServiceName $services.ServiceName -ServiceCredential $cred -ErrorVariable $errVar -WarningVariable $warnVar
    ...
    Context "Change password of the service account" {
        ...
        $errVar = $warnVar = $null
        $results = $services | Sort-Object ServicePriority | Update-SqlServiceAccount -Password $newSecurePassword -ErrorVariable $errVar -WarningVariable $warnVar
        ...
        $results = Get-SqlService -ComputerName $computerName -ServiceName $services.ServiceName | Restart-SqlService
    ...
    Context "Change agent service account to local system" {
        $errVar = $warnVar = $null
        $results = $services | Where-Object { $_.ServiceType -eq 'Agent' } | Update-SqlServiceAccount -Username 'NT AUTHORITY\LOCAL SYSTEM' -ErrorVariable $errVar -WarningVariable $warnVar
    ...
    Context "Revert SQL Agent service account changes ($currentAgentUser)" {
        $errVar = $warnVar = $null
        $results = $services | Where-Object { $_.ServiceType -eq 'Agent' } | Update-SqlServiceAccount -Username $currentAgentUser -ErrorVariable $errVar -WarningVariable $warnVar
    ...
    Context "Revert SQL Engine service account changes ($currentEngineUser)" {
        $errVar = $warnVar = $null
        $results = $services | Where-Object { $_.ServiceType -eq 'Engine' } | Update-SqlServiceAccount -Username $currentEngineUser -ErrorVariable $errVar -WarningVariable $warnVar
